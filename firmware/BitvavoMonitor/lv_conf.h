...
#define LV_STDARG_INCLUDE       <stdarg.h>
    ...
    /* 96 KB internal SRAM was too small once the markets list had >50 rows
     * (each row is 6 LVGL objects + local styles -> pool exhaustion -> hang).
     * Pool is allocated from PSRAM via heap_caps_malloc, so we can be generous. */
     ...
     * On ESP32-S3 we route LVGL's pool to PSRAM (~6.9 MB free) so the UI isn't squeezed into SRAM. */
    #if LV_MEM_ADR == 0
...
#define LV_FONT_CUSTOM_DECLARE LV_FONT_DECLARE(lv_font_jetbrains_mono_24)
...
#define LV_USE_FONT_COMPRESSED 1 /* required for lv_font_conv fonts (bitmap_format compressed) */
